...
Now that we understand the data, we would like to see what the predicted values for all pollution metrics in Spring under the assumption that COVID-19 had not impacted anything. We first convert the data into a time series by months to better capture large scale changes.   

```{r}
# Monthly 
measurement_summary_ts <- ts(measurement_summary_wide, start = c(2017, 1, 1), end = c(2019, 12, 31), frequency = 12)
```
 basically bwant to setup the time series object to do stuff with hourly or maybe dailies


something about vector auto regression model including equation and how similar it is to multi variate linear modeling - the timepointing
check if data is stationary
then do a auto regression model or nonlinear version
predict plot
